...
def test_priyas_email_ends_in_a_proposal_to_look_up_the_order():
    ...
    state = run_graph(build_graph(model), SUBJECT, BODY)
...
def test_a_status_question_skips_extraction():
    ...
    state = run_graph(build_graph(model), "Where is #5102?", "When does it arrive?")
...
def test_the_graph_keeps_no_state_of_its_own():
    ...
    assert build_graph(ScriptedModel()).checkpointer is None
